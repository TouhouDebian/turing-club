...
const guides = {
	"00-cybersecurity-club-trial": {
		...
		enLead: "This trial lesson is a roadmap for the club: web pages, backend ideas, file and memory reading, virtual machines, and a baseline challenge.",
		goals: ["了解社团的学习节奏与实践方式", "知道网页、后端和系统实验之间的关系", "建立合法、可控、可复现的实验意识"],
		sections: [
			section("社团学习路线", "Club learning path", [1, 5], [
			...
			section("从网页到后端", "From webpage to backend", [6, 12], [
			...
			section("实验环境与自测", "Lab environment and self-check", [13, 18], [
		...
		practice: ["安装一个虚拟机并记录快照步骤", "画出一次网页上传到后端处理的流程图", "写下三个你认为最容易出错的信任边界"],
	},
	"01-windows-basics": {
		lead: "Windows 基础课关注的是日常系统如何成为安全实验对象：文件、用户、权限、进程、命令行和网络配置都不是孤立知识，它们共同决定一台主机是否可管理、可审计、可防护。",
		enLead: "This lesson treats Windows as a security target: files, users, permissions, processes, shells, and networking form the base of host security.",
		goals: ["熟悉 Windows 文件与账户体系", "理解权限、进程和服务的基本含义", "会用命令行观察系统状态"],
		sections: [
			section("把 Windows 当作一台主机来理解", "Windows as a host", [1, 3], [
			...
			section("文件、账户与权限", "Files, accounts, and permissions", [4, 8], [
			...
			section("命令行观察与安全排查", "Command-line observation", [9, 12], [
		...
		practice: ["查看当前用户权限并解释结果", "列出一个目录的权限信息", "观察当前网络连接并判断哪些属于浏览器或系统服务"],
	},
	"02-linux-basics": {
		lead: "Linux 基础课把发行版、Shell、文件系统、SSH、用户和权限串起来。它的重点不是记住某个命令，而是理解命令为什么能控制系统资源。",
		enLead: "Linux basics connect distributions, shell, filesystems, SSH, users, and permissions. The point is to understand why commands control resources.",
		goals: ["理解 Linux 发行版与 Shell 的关系", "掌握常见文件和目录操作", "理解 SSH、用户、组和权限的安全意义"],
		sections: [
			section("Linux 与发行版", "Linux and distributions", [1, 5], [
			...
			section("Shell 与命令行", "Shell and command line", [6, 11], [
			...
			section("SSH 与远程连接", "SSH and remote access", [12, 12], [
			...
			section("文件系统、用户与权限", "Filesystem, users, and permissions", [13, 21], [
		...
		practice: ["用命令查看当前 Shell 和当前目录", "创建一个文件并修改权限", "尝试用 SSH 连接实验机并记录认证方式"],
	},
	"03-python-basics": {
		lead: "Python 基础课服务于后续自动化、安全脚本和数据处理。初学时应优先理解变量、流程控制、函数和调试，而不是急着写很长的程序。",
		enLead: "Python basics support automation, security scripting, and data processing. Variables, control flow, functions, and debugging come first.",
		goals: ["理解 Python 程序的基本结构", "能用变量、条件和循环表达简单逻辑", "为后续安全自动化打基础"],
		sections: [
			section("从脚本开始", "Starting from scripts", [1, 6], [
			...
			section("变量、判断与循环", "Variables, branches, and loops", [7, 15], [
			...
			section("函数与模块化", "Functions and modularity", [16, 24], [
			...
			section("练习与调试", "Practice and debugging", [25, 33], [
		...
		practice: ["写一个脚本统计文本中某个关键词出现次数", "用函数封装一次日志行解析", "故意制造一个错误并解释报错位置"],
	},
	"04-data-conversion-basics": {
		lead: "数据转换基础课解决一个常见困惑：安全题目中的字符串为什么会变成二进制、十六进制、Base64 或 JSON。理解编码不是炫技，而是为了还原数据本来的含义。",
		enLead: "Data conversion explains why strings appear as binary, hex, Base64, Unicode, or JSON in security tasks.",
		goals: ["区分编码、加密和哈希", "理解常见进制与字符集", "能在 CTF 或日志中还原数据"],
		sections: [
			section("为什么要做数据转换", "Why conversion matters", [1, 3], [
			...
			section("进制与字符集", "Bases and character sets", [4, 8], [
			...
			section("Base64、JSON 与实战还原", "Base64, JSON, and recovery", [9, 13], [
		...
		practice: ["把一段文本转换为十六进制再还原", "判断一段 Base64 文本解码后是什么格式", "解释编码、加密、哈希三者区别"],
	},
	"05-cryptography-basics": {
		lead: "密码学基础课介绍从古典密码到现代加密的核心思想。课堂目标不是手写完整算法，而是知道不同密码机制解决什么问题、不能解决什么问题。",
		enLead: "Cryptography basics introduce classical and modern ideas: confidentiality, integrity, authentication, and their limits.",
		goals: ["理解对称加密、非对称加密和哈希的区别", "认识 Vigenere、XOR 等基础思想", "建立不要自创密码算法的安全意识"],
		sections: [
			section("密码学解决什么问题", "What cryptography solves", [1, 4], [
			...
			section("对称与非对称加密", "Symmetric and asymmetric encryption", [5, 7], [
			...
			section("Vigenere 与 XOR", "Vigenere and XOR", [8, 14], [
			...
			section("作业与复习", "Homework and review", [15, 17], [
		...
		practice: ["用自己的话解释哈希和加密的区别", "完成一个简单 XOR 还原练习", "列出 TLS 中至少两个密码学机制"],
	},
	"06-firewall-basics": {
		lead: "防火墙基础课讨论流量如何被允许、拒绝、记录和分段。它不是一道万能墙，而是一套基于规则、场景和日志的访问控制方法。",
		enLead: "Firewall basics explain how traffic is allowed, denied, logged, and segmented through rules and policy.",
		goals: ["理解防火墙在网络防御中的位置", "认识端口、协议、方向和规则顺序", "能用日志复盘一次访问控制结果"],
		sections: [
			section("防火墙的角色", "Role of a firewall", [1, 7], [
			...
			section("规则、端口与方向", "Rules, ports, and direction", [8, 12], [
			...
			section("示例与作业", "Examples and homework", [13, 18], [
		...
		practice: ["写出一条允许 SSH 的规则条件", "解释入站和出站规则的区别", "设计一个默认拒绝的最小开放策略"],
	},
	"07-network-basics": {
		lead: "网络基础课从“送快递”的比喻讲起，把 TCP/IP、常见协议、流量分析和 VPN 串成一张地图。安全学习离不开网络，因为攻击和防御都需要在流量中留下痕迹。",
		enLead: "Networking basics connect TCP/IP, protocols, traffic analysis, and VPN through the idea of delivering packets.",
		goals: ["理解分层模型和 TCP/IP 的基本作用", "区分 TCP、UDP 与常见应用层协议", "知道流量分析能观察到什么"],
		sections: [
			section("网络像送快递", "Networking as delivery", [1, 6], [
			...
			section("TCP、UDP 与应用层协议", "TCP, UDP, and application protocols", [7, 10], [
			...
			section("流量分析与辅助技术", "Traffic analysis and supporting technologies", [11, 16], [
		...
		practice: ["解释 TCP 和 UDP 的差异", "抓一次 DNS 查询并标出请求和响应", "画出访问一个网站时可能经过的协议"],
	},
	"08-attack-techniques": {
		lead: "攻击技术简介课用于建立攻防视角：攻击链通常包含信息收集、漏洞利用、权限扩大、维持访问和清理痕迹等环节。学习这些内容的目的，是更好地理解防御重点。",
		enLead: "This lesson introduces attack stages so learners can understand what defenders need to observe and prevent.",
		goals: ["认识常见攻击链阶段", "理解攻击技术与防御措施的对应关系", "强化授权实验和安全伦理"],
		sections: [
			section("攻击链不是一个按钮", "An attack chain is not one button", [1, 4], [
			...
			section("从侦察到利用", "From reconnaissance to exploitation", [5, 9], [
			...
			section("权限、维持与防御", "Privilege, persistence, and defense", [10, 13], [
		...
		practice: ["用表格列出攻击链每一步对应的防御措施", "选择一个漏洞案例并写出前提条件", "说明为什么授权范围比工具本身更重要"],
	},
	"09-information-gathering-part-1": {
		lead: "信息收集与枚举上半部分先讲被动信息收集：尽量不直接接触目标系统，通过公开资料、搜索引擎、域名和网页痕迹建立目标画像。",
		enLead: "Part 1 focuses on passive reconnaissance: building a target profile from public information before touching the target.",
		goals: ["理解信息收集的目的和边界", "掌握被动收集的常见来源", "知道如何记录证据和不足"],
		sections: [
			section("信息收集的意义", "Purpose of reconnaissance", [1, 4], [
			...
			section("被动信息收集", "Passive reconnaissance", [5, 10], [
			...
			section("不足与下一步", "Limitations and next steps", [11, 19], [
		...
		practice: ["为一个授权靶场域名整理公开信息来源", "写出三条搜索语句及其目的", "把收集结果分成事实、推测、待验证三类"],
	},
	"09-information-gathering-enumeration": {
		lead: "完整的信息收集与枚举课程在被动收集之后加入主动枚举、自动化和作业设计。重点是把零散线索转成可验证的资产清单。",
		enLead: "The full lesson moves from passive reconnaissance to active enumeration and automation, turning clues into an asset inventory.",
		goals: ["区分被动收集与主动枚举", "理解 DNS、端口和服务发现的作用", "建立自动化但不过度打扰目标的意识"],
		sections: [
			section("从线索到资产", "From clues to assets", [1, 8], [
			...
			section("搜索引擎与被动资料", "Search engines and passive sources", [9, 10], [
			...
			section("主动枚举与自动化", "Active enumeration and automation", [11, 14], [
			...
			section("作业与复盘", "Homework and review", [15, 17], [
		...
		practice: ["建立一份资产清单模板", "对授权目标做一次 DNS 记录整理", "解释自动化扫描为什么需要限速"],
	},
	"10-understanding-web-attacks": {
		lead: "Web 攻击入门课围绕数据库、SQL 注入和 XSS 展开。它的核心是理解用户输入如何进入服务器、数据库和浏览器，并在错误处理时造成安全影响。",
		enLead: "This web security lesson focuses on databases, SQL injection, and XSS: how input moves through server, database, and browser.",
		goals: ["理解 Web 应用的输入输出路径", "认识 SQL 注入和 XSS 的基本原理", "知道常见防御方向"],
		sections: [
			section("Web 应用与数据库", "Web apps and databases", [1, 7], [
			...
			section("SQL 注入", "SQL injection", [8, 10], [
			...
			section("XSS 与浏览器执行", "XSS and browser execution", [11, 12], [
			...
			section("作业与安全边界", "Homework and boundaries", [13, 15], [
		...
		practice: ["画出登录表单到数据库查询的数据流", "说明参数化查询如何防 SQL 注入", "比较反射型和存储型 XSS"],
	},
	"11-web-penetration": {
		lead: "Web 渗透课程以 Netcat 和考试式练习为入口，强调工具只是手段，真正要训练的是网络连接、输入输出、证据记录和边界意识。",
		enLead: "This lesson uses Netcat and exam-style practice to train connections, input/output, evidence, and boundaries.",
		goals: ["理解 Netcat 的常见用途", "认识反弹 shell 和文件传输的风险", "为 Web 安全综合练习做准备"],
		sections: [
			section("课程结构与边界", "Course structure and boundaries", [1, 2], [
			...
			section("Netcat 的用途", "What Netcat is used for", [3, 8], [
			...
			section("考试式练习", "Exam-style practice", [9, 13], [
			...
			section("未来的 Web 安全学习", "Future web security learning", [14, 15], [
		...
		practice: ["用 Netcat 在本地监听并发送一段文本", "解释反弹 shell 为什么危险", "写一段包含证据和修复建议的小报告"],
	},
	"12-defending-injection-attacks": {
		lead: "防御注入攻击课程从攻击原理转向修复方法：不要让用户输入直接改变命令、查询或解释器语义。核心防线是参数化、校验、最小权限和持续维护。",
		enLead: "Defending injection means preventing user input from changing query, command, or interpreter semantics.",
		goals: ["理解注入类漏洞的共同结构", "掌握参数化查询和输入校验思路", "认识后期维护的重要性"],
		sections: [
			section("注入攻击的共同点", "Common pattern of injection", [1, 5], [
			...
			section("防御概览", "Defense overview", [6, 8], [
			...
			section("代码示例与维护", "Code examples and maintenance", [9, 12], [
		...
		practice: ["把一段拼接 SQL 改成参数化查询思路", "列出命令注入的三个防御点", "写一个修复验证清单"],
	},
	"13-defending-password-bruteforce": {
		lead: "防御密码爆破课程关注认证系统如何抵抗大量猜测。重点不是让密码规则越来越复杂，而是组合限速、锁定、MFA、监控和用户体验。",
		enLead: "Defending brute force attacks combines rate limiting, lockout, MFA, monitoring, and usable authentication design.",
		goals: ["理解爆破攻击的基本条件", "掌握限速、锁定和 MFA 的作用", "知道如何监控认证异常"],
		sections: [
			section("爆破攻击如何发生", "How brute force works", [1, 5], [
			...
			section("防御手段", "Defense controls", [6, 8], [
			...
			section("代码、日志与维护", "Code, logs, and maintenance", [9, 12], [
		...
		practice: ["设计一个登录限速策略", "解释 MFA 能防什么、不能防什么", "列出认证日志中应记录的字段"],
	},
	"14-defending-xss-attacks": {
		lead: "防御 XSS 课程把重点放在输出上下文、编码、CSP 和安全开发习惯上。XSS 防御不是简单删除 `<script>`，而是系统性管理不可信内容。",
		enLead: "Defending XSS requires output-context encoding, CSP, secure rendering, and careful handling of untrusted content.",
		goals: ["理解三类 XSS 的触发位置", "掌握输出编码和 CSP 的作用", "形成前端安全开发习惯"],
		sections: [
			section("XSS 的攻击原理", "How XSS works", [1, 5], [
			...
			section("防御概览", "Defense overview", [6, 7], [
			...
			section("代码示例与复盘", "Code examples and review", [8, 12], [
		...
		practice: ["比较反射型、存储型和 DOM 型 XSS", "为评论区设计 XSS 防御方案", "解释 CSP 为什么不是唯一防线"],
	},
	"15-handover-permissions": {
		lead: "交接与权限处理课偏向社团运营和工程习惯：任务交付、权限移交、文档记录、AI 使用边界和活动收尾，都会影响项目能否长期运转。",
		enLead: "This operations lesson covers task handover, permission transfer, documentation, AI boundaries, and sustainable club workflow.",
		goals: ["理解任务交付和权限交接的重要性", "建立文档和审计意识", "明确 AI 工具使用边界"],
		sections: [
			section("任务不是做完就结束", "A task is not done until handed over", [1, 5], [
			...
			section("权限与自由", "Permissions and freedom", [6, 9], [
			...
			section("活动收尾", "Closing an activity", [10, 12], [
		...
		practice: ["写一份项目交接清单", "列出一个仓库需要哪些角色权限", "设计离任撤权流程"],
	},
	"cynthia-li-honeypots": {
		lead: "蜜罐课程从主动防御角度介绍如何用诱饵系统收集威胁情报。它把传统蜜罐、IoT 蜜罐、云蜜罐、AI 自适应防御和未来趋势连接起来。",
		enLead: "This honeypot lesson explains deception-based active defense, from traditional systems to IoT, cloud, AI, and future trends.",
		goals: ["理解蜜罐的定义和价值", "区分不同交互程度和部署类型", "认识 AI 与蜜罐结合的机会和风险"],
		sections: [
			section("蜜罐为什么是主动防御", "Why honeypots are active defense", [1, 2], [
			...
			section("分类与交互程度", "Types and interaction levels", [3, 4], [
			...
			section("AI、IoT 与分布式蜜网", "AI, IoT, and distributed honeynets", [5, 8], [
			...
			section("历史、挑战与未来", "History, challenges, and future", [9, 12], [
		...
		practice: ["比较低交互和高交互蜜罐", "设计一个 IoT 蜜罐要模拟的协议", "列出蜜罐部署的三个风险控制点"],
	},
	"linux-ai-vulnerability-walkthrough": {
		...
		enLead: "This walkthrough uses Dirty Pipe, Zero Copy, and AI scanning to explain why optimized systems can still hide deep kernel bugs.",
		goals: ["理解 Linux 内核漏洞为什么影响巨大", "认识 Zero Copy 和 Dirty Pipe 的基本概念", "讨论 AI 在漏洞发现中的作用和限制"],
		sections: [
			section("漏洞时间线与 Linux 安全", "Timeline and Linux security", [1, 4], [
			...
			section("Zero Copy、Dirty Pipe 与漏洞成因", "Zero Copy, Dirty Pipe, and root cause", [5, 10], [
			...
			section("AI 如何参与漏洞发现", "How AI helps vulnerability research", [11, 12], [
			...
			section("影响、比喻与讨论", "Impact, analogy, and discussion", [13, 19], [
		...
		practice: ["解释 Zero Copy 为什么能提升性能", "用三句话说明 Dirty Pipe 的风险", "讨论 AI 扫描漏洞需要哪些人工验证"],
	},
};
...
const collectSlideImages = (body) => {
	const slides = new Map();
	const imageMatches = [...body.matchAll(/!\[[^\]]*\]\((\.\/images\/slide-(\d+)-[^)]+)\)/g)];
	for (const match of imageMatches) {
		const src = match[1];
		const slide = Number(match[2]);
		if (!slides.has(slide)) slides.set(slide, []);
		slides.get(slide).push(src);
	}
	return slides;
};

const renderImagesForRange = (slides, range, usedSlides) => {
	...
	for (let slide = start; slide <= end; slide += 1) {
		...
		lines.push(`### 第 ${slide} 页配图 / Slide ${slide} Images`, "");
		for (const src of images) lines.push(`![Slide ${slide} image](${src})`);
		lines.push("");
	}
	return lines.join("\n").trim();
};

const renderLeftoverImages = (slides, usedSlides) => {
	const lines = [];
	for (const slide of [...slides.keys()].sort((a, b) => a - b)) {
		if (usedSlides.has(slide)) continue;
		const images = slides.get(slide);
		if (!images?.length) continue;
		lines.push(`### 第 ${slide} 页配图 / Slide ${slide} Images`, "");
		for (const src of images) lines.push(`![Slide ${slide} image](${src})`);
...
const renderKeywords = (tags) => tags.map((tag) => `#${tag.replace(/\s+/g, "-")}`).join(" ");
...
for (const [slug, guide] of Object.entries(guides)) {
	...
	const author = original.match(/\*\*作者 \/ Author:\*\*\s*(.+)/)?.[1]?.trim() ?? "";
	const date = original.match(/\*\*原 PPT 日期 \/ Original PPT date:\*\*\s*(.+)/)?.[1]?.trim() ?? frontmatter.published;
	const slides = collectSlideImages(frontmatter.body);
	...
	const lines = [
		...
		`# ${titleZh}`,
		"",
		titleEn ? `**English title:** ${titleEn}` : "",
		"",
		author ? `**作者 / Author:** ${author}` : "",
		"",
		`**原 PPT 日期 / Original PPT date:** ${date}`,
		"",
		`**关键词 / Keywords:** ${renderKeywords(tags)}`,
		"",
	...
	guide.sections.forEach((item, index) => {
		lines.push(`## ${index + 1}. ${item.title} / ${item.enTitle}`, "");
		for (const paragraph of item.paragraphs) lines.push(paragraph, "");
		lines.push(`> English recap: ${item.enSummary}`, "");
		const renderedImages = renderImagesForRange(slides, item.slides, usedSlides);
		if (renderedImages) lines.push("### 相关课件图片 / Related Slide Images", "", renderedImages, "");
	});

	lines.push("## 课堂练习 / Practice", "", renderList(guide.practice), "");
	const leftover = renderLeftoverImages(slides, usedSlides);
	if (leftover) {
		lines.push(
			"## 原始课件补充图片 / Additional Original Slide Images",
			"",
			"以下图片来自原 PPT 中未被上文单独归入正文段落的页面，保留用于查阅原始视觉材料。",
			"",
			"The following images are preserved from slides that were not separately attached to a narrative section above.",
			"",
			leftover,
			"",
		);
	...
	writeFileSync(postPath, `${lines.filter((line) => line !== undefined).join("\n").replace(/\n{3,}/g, "\n\n").trim()}\n`, "utf8");
}
